...
Subject: Request to Close Licious Account – Shipment ID: [id]
...
Apologise for inconvenience:
"Its showing our DE tried to reach you several times but due to not proper call connectivity, he had to cancel the order."
In case of prepaid order, refund will be initiated to source TAT 3 to 5 working days, in Licious wallet 24 hours.

tagging - cancellation related --> order cancelled without consent
`,
          "Order Rescheduled-li Fact": `The order was rescheduled due to bandh, DE not available, hub infrastructure, etc. Apologies for inconvenience.
...
`,
          "Order Modification-li Fact": `Imagine you ordered 2 chicken; one scheduled for today and one for tomorrow.
Check the reason of modification [Product OFS / Damaged cases].
If shipment is ongoing and customer wants to cancel due to order modification, cancel the entire order.

tagging - cancellation related --> order cancellation - licious factors --> order modification
`,
          "Delivery is Delayed-li Fact": `If order got delayed and customer is asking "Where is my order?"
Check status if "Processing / Accepted / Billed".
Call hub or AM - escalate ASAP if no response.
If customer not convinced with TAT, offer initial refund.

tagging - cancellation related --> order cancellation - licious factors --> delivery is delayed
`,
          "Changed mind [ssi]-cutomer factor": `Customer decided to cancel the entire order [he/she changed their mind].
[Cancel fee applies].

Tagging - Cancellation related --> customer factors --> change mind
`,
          "Add/remove item-cutomer factor": `We cannot add items; we only remove items and cancel [cancel fee applies].

Tagging - Cancellation related --> customer factors --> forgot to add/remove item
`,
          "Forgot to apply coupon-cutomer factor": `We can apply a coupon on behalf of customer [before billed].
After bill, if customer wants to cancel the order [cancel fee applies].

Tagging - Cancellation related --> customer factors --> forgot to apply coupon
`,
          "Order got split-cutomer factor": `The order already got split by the app.
Now customer wants to cancel the order [before billed].

Tagging - Cancellation related --> customer factors --> order split in different slot
`,
          "Preferred slot unavailable-cutomer factor": `Customer placed an order but wants to change it to a different slot which is not available.

Tagging - Cancellation related --> customer factors --> slot unavailable to reschedule
`,
          "Customer Not available-cutomer factor": `Customer is not available to take the order, so they cancel the order.

Tagging - Cancellation related --> customer factors --> not available to receive order
`,
          "Chose a wrong Address-cutomer factor": `Customer chose a wrong address by mistake; they cancel the order.

Tagging - Cancellation related --> customer factors --> chose a wrong address
`,
          "Payment Failed-cutomer factor": `Due to some technical issue, the payment failed.
...
Apologise to customer… assure that we will work on this and this won’t happen again.
...
Apologise to customer, ask for exact landmark and address and inform DE.

Grievance in Delivery related --> too many calls made to locate address.

Email - hub, tlchc

Tagging - delivery related --> DE complaint --> too many calls to address

* Did not deliver to doorstep

Apologise to customer and ask if DE allowed in your society or flat (if yes, take as feedback and send email to hub).

Email - hub, tlchc

Tagging --> delivery related --> DE complaint --> did not deliver to doorstep`,
...
A customer contacted CHC and complained that a [iteam name ] in the order were missing. but cusotmer want that product so try to delivered it ASAP
Customer Registered phone number:
Shipment Id:
Missing Products:
...
A customer contacted CHC and complained that a [iteam name ] in the order were missing. We have issued a partial refund for the customer as per their request; kindly notify the Delivery Hero not to deliver the product to the customer.
...
Raise Grievance in Workbench with category Smell Issues.`
    ...
    subGroupSelect.addEventListener("change", () => {
      ...
      if (mainGroup && subGroup) {
        instructionDiv.innerHTML = data[mainGroup].subgroups[subGroup]
          .replace(/\bli factor\b/g, '<span class="li-factor">li factor</span>')
          .replace(/\bcustomer factor\b/g, '<span class="customer-factor">customer factor</span>');
      } else {
        instructionDiv.textContent = "";
      }
    });
  ...
  function populateSubGroups(mainGroup) {
    subGroupSelect.innerHTML = '<option value="">-- Select Sub Group --</option>';
    if (mainGroup && data[mainGroup]) {
      Object.keys(data[mainGroup].subgroups).forEach(sub => {
        const option = document.createElement("option");
        option.value = sub;
        option.textContent = sub;
        subGroupSelect.appendChild(option);
  ...
  subGroupSelect.addEventListener("change", function () {
    ...
    if (mainGroup && subGroup) {
      instructionDiv.textContent = data[mainGroup].subgroups[subGroup] || "No instructions available.";
    } else {
      instructionDiv.textContent = "";
    }
  });
  ...
  document.getElementById("calculateBtn").addEventListener("click", () => {
    ...
    const packageOption = parseInt(document.getElementById("packageOption").value, 10);
    const damagedEggs = parseInt(document.getElementById("damagedEggs").value, 10);

    if (isNaN(orderValue) || orderValue <= 0) {
      alert("Please enter a valid positive order value.");
      return;
    }
    if (![6, 12, 30].includes(packageOption)) {
      alert("Please select a valid package option.");
      return;
    }
    if (isNaN(damagedEggs) || damagedEggs < 0) {
      alert("Please enter a valid number of damaged eggs.");
      return;
    }

    const damageAmount = (orderValue / packageOption) * damagedEggs;
    document.getElementById("calc-result").textContent = `Damage Amount: ₹${damageAmount.toFixed(2)}`;
  });

  // egg calculator logic
  document.getElementById("calculateBtn").addEventListener("click", () => {
    ...
    if (isNaN(orderValue) || orderValue <= 0) {
      resultDiv.textContent = "Please enter a valid positive order value.";
      return;
    }
    if (![6, 12, 30].includes(packageOption)) {
      resultDiv.textContent = "Please select a valid package option.";
      return;
    }
    if (isNaN(damagedEggs) || damagedEggs < 0) {
      resultDiv.textContent = "Please enter a valid number of damaged eggs.";
      return;
    }
    ...
    if (packageOption === 6) {
      refundAmount = damagedEggs >= 1 ? orderValue :  ;
    ...
    resultDiv.textContent = `Refund Amount: ₹${refundAmount.toFixed(2)}`; });