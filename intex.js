    ...
    // 🔥 NEW: Save session to sessionStorage
    function saveSession(user) {
      try {
        sessionStorage.setItem('amastor_user', JSON.stringify(user));
        sessionStorage.setItem('amastor_login_time', Date.now().toString());
      } catch (e) {
        console.log('Session storage not available');
      }
    }

    // 🔥 NEW: Load session from sessionStorage
    function loadSession() {
      try {
        const userStr = sessionStorage.getItem('amastor_user');
        const loginTime = sessionStorage.getItem('amastor_login_time');
    ...
    function clearSession() {
      try {
        sessionStorage.removeItem('amastor_user');
        sessionStorage.removeItem('amastor_login_time');
      } catch (e) {
    ...
    document.getElementById('verifyOtpBtn').addEventListener('click', function() {
      ...
      if (enteredOTP === currentOTP) {
        ...
        // 🔥 NEW: Save session for persistence
        saveSession(user);
